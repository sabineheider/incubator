...
public class DynamicClassWriter {
	...
	public byte[] writeClass(Class parentClass, String className) {
		if (parentClass == null || parentClass.isPrimitive()
				...
				|| Modifier.isFinal(parentClass.getModifiers())) {
		...
		cw.visit(V1_2, ACC_PUBLIC + ACC_SUPER, className.replace('.', '/'),
				Type.getType(parentClass).getInternalName(), getInterfaces(),
				null);
	 ...
	 * to support {@link java.io.Serializable} parent classes.
	 */
	protected void addWriteReplace(ClassWriter cw, Class parentClass) {
		...
		if (Serializable.class.isAssignableFrom(parentClass)
			...
			String[] exceptionsDesc = new String[] { Type.getType(
					ObjectStreamException.class).getInternalName() };

			CodeVisitor mv = cw.visitMethod(ACC_PROTECTED, method.getName(),
					methodDesc, exceptionsDesc, null);
			mv.visitVarInsn(ALOAD, 0);
			mv.visitMethodInsn(INVOKESPECIAL,
					Type.getInternalName(parentClass), method.getName(),
					methodDesc);
